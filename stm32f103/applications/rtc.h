...
/* 接口函数 */
void rtc_sec2date(uint8_t *time,uint32_t sec);          /*总秒数转换成日期数组*/
uint8_t Bin2BCD(uint8_t bin);                       /*二进制转换为BCD码*/
uint8_t rtc_init(void);                             /* 初始化RTC */
void rtc_get_time(void);                            /* 获取RTC时间信息 */
uint32_t rtc_get_seccnt(void);                      /*获取RTC总秒数*/
...
uint8_t rtc_set_sec_alarm(uint32_t sec); /* 设置闹钟时间,总秒数 */

uint8_t rtc_set_gap_alarm(uint32_t gap_sec);/*设置闹钟时间为当前时间往后的gap_sec秒*/
