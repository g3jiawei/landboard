...
uint8_t rtc_init(void)
{
    ...
    {
        rtc_set_time(2024, 4, 15, 15, 23, 35);              /* 设置时间 */
    }
...
uint8_t rtc_set_sec_alarm(uint32_t sec)
{
    __HAL_RCC_PWR_CLK_ENABLE(); /* 使能电源时钟 */
    __HAL_RCC_BKP_CLK_ENABLE(); /* 使能备份域时钟 */
    HAL_PWR_EnableBkUpAccess(); /* 取消备份域写保护 */
 ...
 *              合法年份范围为: 1970 ~ 2105年
 * @param       time[] :存储时间的数组
 * @param       以1970年1月1日为基准的秒数
 */

void rtc_sec2date(uint8_t *time,uint32_t sec){
        uint16_t daycnt = 0;
        uint32_t temp = 0;
        uint16_t temp1 = 0;
    ...
    if (daycnt != temp)         /* 超过一天了 */
    {
        ...
        time[0] = temp1-2000;  /* 得到年份 ,temp1的低8位存入time[0]*/
        temp1 = 0;
    ...
    time[3] = temp / 3600;                                                /* 小时 */
    time[4] = (temp % 3600) / 60;                                          /* 分钟 */
    time[5] = (temp % 3600) % 60;                                          /* 秒钟 */
...
//    seccount <<= 16;
//    seccount += RTC->CNTL;
    seccount = rtc_get_seccnt(); 
    rtc_sec2date(time,seccount);
    
    calendar.year   = time[0];                                      /* 得到年份 */
    calendar.month  = time[1];                                      /* 得到月份 */
    calendar.date   = time[2];                                      /* 得到日期 */
    calendar.hour   = time[3];                                      /* 小时 */
    calendar.min    = time[4];                                      /* 分钟 */
    calendar.sec    = time[5];                                      /* 秒钟 */
    calendar.week = rtc_get_week(calendar.year, calendar.month, calendar.date); /* 获取星期 */
}
...
/**
 * @brief       当前时间基础上+时间间隔生成闹钟
 * @note        以1970年1月1日为基准, 1970年1月1日, 0时0分0秒, 表示第0秒钟
 *              最大表示到2105年, 因为uint32_t最大表示136年的秒钟数(不包括闰年)!
 * @param       gap_sec : 间隔的值，单位为秒
 * @retval      转换后的秒钟数
 */
uint8_t rtc_set_gap_alarm(uint32_t gap_sec)
{
    uint32_t sec_now;
    int16_t err=0;
    sec_now = rtc_get_seccnt();
    sec_now += gap_sec;
    err = rtc_set_sec_alarm(sec_now);
    return err;
}
